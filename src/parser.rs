...
#[wasm_bindgen]
pub struct SplatParser;

#[wasm_bindgen]
impl SplatParser {
    #[wasm_bindgen(constructor)]
    pub fn new() -> SplatParser {
        SplatParser
    }
}

impl SplatParser {
    // NOTE: This currently only parses .ply files with the format "binary"
    // Internal function not exposed to WASM
    pub fn load_ply(path: &Path) -> Result<Vec<RawSplat>> {
        ...
        match format_type {
            ...
            "ascii" => {
                unimplemented!("ASCII format not implemented yet")
            }
            _ => {
